...
from hio_sdf.srv import ShareWeights, ShareWeightsResponse
import io
...
class HIOSDFNode:
    ...
    def init_weight_server(self):
        self.serv = rospy.Service('weights', ShareWeights, self.weight_serv_handle)
        rospy.loginfo("Model weights service ready")
    
    def weight_serv_handle(self, req):
        # Define the file path (DEBUGGING)
        file_path1 = "/home/ros/exchange/weight_data/py_expected_weights.txt"
        file_path2 = "/home/ros/exchange/weight_data/py_sent_weights.txt"

        # Open the file in write mode (DEBUGGING)
        with open(file_path1, 'w') as file:
            # Get the model state dictionary (DEBUGGING)
            model_params = self.global_sdf_trainer.model.state_dict()
            
            # Write each parameter's name and value to the file (DEBUGGING)
            for name, param in model_params.items():
                file.write(f"{name}: {param}\n")

        state_dict = self.global_sdf_trainer.model.state_dict()
    
        # Extract weights as a flat list of float values
        weights = []
        for param in state_dict.values():
            weights.extend(param.flatten().tolist())
        
        # w_buffer = io.BytesIO()
        # torch.save(self.global_sdf_trainer.model.state_dict(), w_buffer)
        # weights_bytes = w_buffer.getvalue()
        
        ## Convert serialized data to a list of uint8 values
        # weights_uint8 = [int(byte) for byte in weights_bytes] 
        ...
        # (DEBUGGING)
        # debug_file = '/home/ros/exchange/weight_data/forwardSave_Python.txt'
        # input_debug_tensor = torch.tensor([196.0, 38.0, 2.0])
        # self.forwardSave(debug_file, input_debug_tensor)
        # input_debug_tensor = torch.tensor([197.0, 38.0, 2.0])
        # self.forwardSave(debug_file, input_debug_tensor)
        # input_debug_tensor = torch.tensor([198.0, 38.0, 2.0])
        # self.forwardSave(debug_file, input_debug_tensor)
        # input_debug_tensor = torch.tensor([196.0, 39.0, 2.0])
        # self.forwardSave(debug_file, input_debug_tensor)
        # input_debug_tensor = torch.tensor([197.0, 39.0, 2.0])
        # self.forwardSave(debug_file, input_debug_tensor)
        # input_debug_tensor = torch.tensor([198.0, 39.0, 2.0])
        # self.forwardSave(debug_file, input_debug_tensor)

        # Save model
        model_copy = self.global_sdf_trainer.model
        #CPU_model = model_copy.to('cpu').eval()
        scripted_model = torch.jit.script(model_copy)
        torch.jit.save(scripted_model, '/home/ros/exchange/weight_data/model.pt')
        #PUblish empty message when updating the network
        empty_msg = Empty()
        self.net_update_publisher.publish(empty_msg)


        # Send the response
        return ShareWeightsResponse(weights=weights)

    def forwardSave(self, filename, input_tensor):
        with open(filename, 'a') as file:
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            input_tensor = input_tensor.to(device)
            output_tensor = self.global_sdf_trainer.model.forward(input_tensor)
            file.write(f"Net value for (x,y,z) = {input_tensor} ==> {output_tensor}")
    ...
    def init_publishers(self):
        ...
        self.net_update_publisher = rospy.Publisher('/net_update', Empty, queue_size = 1)
    ...
    def pointcloud_callback(self, pointcloud_msg):
        ...
        np.savez_compressed(
            "/home/ros/exchange/voxfield_esdf_map.npz",
            indices=indices.astype(np.int32),     # block indices (N x 3)
            sdf=np.asarray(sdf, dtype=np.float32), # flattened ESDF values
            observed=observed.astype(np.uint8),   # voxel observed flags
            block_size=np.float32(self.block_size),
            voxels_per_side=np.int32(self.voxel_per_side)
        )
        ...
        # Update saved model and send empty message as update notice
        model_copy = self.global_sdf_trainer.model
        scripted_model = torch.jit.script(model_copy)
        torch.jit.save(scripted_model, '/home/ros/exchange/weight_data/model.pt')
        empty_msg = Empty()
        self.net_update_publisher.publish(empty_msg)
        rospy.loginfo(f"===============================================================================")
        rospy.loginfo(f"==========================PUBLISHED NEW NETWORK STATE==========================")
        rospy.loginfo(f"===============================================================================")
    ...
    def publish_data_slice(self, stamp):
        ...
        slice_idx = np.logical_and(sdf_data[:, 2] < 2, sdf_data[:, 2] > 0)
        ...
        self.data_slice_publisher.publish(pc_msg) # Publica en /data_slice
